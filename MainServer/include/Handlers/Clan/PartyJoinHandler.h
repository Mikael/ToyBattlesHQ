...
namespace Main
{
    namespace Handlers
    {
        ...
        // Reviewed 6/02/2025
        inline void handlePartyJoin(const Common::Network::Packet& request, std::shared_ptr<Main::Network::Session> session, Main::Classes::ClansManager& clansManager,
            Main::Classes::RoomsManager& roomsManager)
        {
            ...
            else if (auto* selfClanRoom = clansManager.getExactRoomFor(ainfo.clanId, requestStructure.roomNumber))
            {
                session->sendMessage("Joined party room number: " + std::to_string(requestStructure.roomNumber), Main::Enums::TIP);

                auto leaderSession = selfClanRoom->getLeaderSession(); 
                if (!leaderSession)
                {
                    session->sendMessage("Server error: Leader's session was not found. If this issue persists, please report it to the team.");
                    response.setExtra(ClanJoinExtra::CLAN_JOIN_GENERAL_ERROR);
                    session->asyncWrite(response);
                    return;
                }
                ...
                // Get players who are already inside the clan room
                const auto& waitingPlayers = selfClanRoom->getWaitingPlayers();
                ...
                selfClanRoom->broadcastToWaitingPlayersExceptSelf(response, session->getAccountInfo().uniqueId.session);
