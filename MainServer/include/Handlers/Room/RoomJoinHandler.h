...
namespace Main
{
	namespace Handlers
	{
		...
		inline void handleRoomJoin(const Common::Network::Packet& request, std::shared_ptr<Main::Network::Session> session, Main::Classes::RoomsManager& roomsManager,
			bool roomJoinCheckPassword = false, std::optional<Main::ClientData::RoomInfo> followUserRoomInfo = std::nullopt, bool joinInvisible = false)
		{
			START_BENCHMARK
			...
			if (Main::Classes::Room* room = roomsManager.getRoomByNumber(requestStructure.roomNumber + 1))
			{
				...
				if (room->playerExists(session->getId()))
				{
					...
					disconnectionReq.setTcpHeader(session->getId(), Common::Enums::USER_LARGE_ENCRYPTION);
					disconnectionReq.setCommand(73, 0, 27, 0);
					session->asyncWrite(disconnectionReq);
					session->closeSocket();
					return;
				}
				...
				if (!joinInvisible && room->wasPreviouslyKicked(accountInfo.accountID))
				...
				response.setCommand(RoomJoinOrder::RoomInfo, room->getRoomInfo().hasPassword, room->getRoomInfo().roomNumber, 8);
				...
				// Player clan infos; option => again num of players (= num of clans)
				...
				auto allObserverPlayersClans = room->getObserverPlayersClans();
				response.setCommand(RoomJoinOrder::RoomPlayersClans, 1, 37, allObserverPlayersClans.size());
				response.setData(reinterpret_cast<std::uint8_t*>(allObserverPlayersClans.data()), sizeof(Main::Structures::PlayerClan) * allObserverPlayersClans.size());
				session->asyncWrite(response);

				// Success in joining the room
				response.setCommand(request.getOrder(), 0 /* joinInvisible ? 1 : 0*/, (joinedAsObserver || joinInvisible) ? RoomJoinExtra::JOIN_OBSERVER_MODE
					: RoomJoinExtra::JOIN_SUCCESS, 0);
				...
				if (joinedAsObserver || joinInvisible)
				{
				...
				response.setCommand(RoomJoinOrder::RoomLatestEnteredPlayerInfo, 0, 0, 1); // Mission=1 ==> Invisible Mode
				...
				if (accountInfo.clanId > 8)
				{
					...
					playerClan.playerRoomIdx = joinedAsObserver ? room->getObserversSize() : room->getPlayersSizeNoObserver();
					response.setCommand(RoomJoinOrder::RoomPlayersClans, joinedAsObserver, 37, 1);
			...
			END_BENCHMARK(handleRoomJoin, session)
		}
	}
}
