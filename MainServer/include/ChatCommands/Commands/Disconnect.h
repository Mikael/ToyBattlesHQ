...
namespace Main
{
	namespace Command
	{
		class Disconnect final : public ICommand
		{
		private:
		...
		public:
			explicit Disconnect(const Common::Enums::PlayerGrade requiredGrade)
				: ICommand{ requiredGrade, "/disconnect <nickname>",  R"(^\S+\s+(.*)$)" }
			{
			}
				...
				Main::MainServer&) override 
			{
				...
				{
					session->sendMessage("error: The target player has equal or greater grade than you.");
					return;
				}
		...
		REGISTER_CMD(Disconnect, Common::Enums::PlayerGrade::GRADE_MOD)
	};
}
