...
namespace Main
{
	namespace Command
	{
		class DisableRoomCreationFor final : public ICommand
		{
		private:
		...
		public:
			explicit DisableRoomCreationFor(const Common::Enums::PlayerGrade requiredGrade)
				: ICommand{ requiredGrade, "/disableroomcreationfor <nickname> <duration>(in days) <reason>",
				R"(^(\S+)\s+(\S+)\s+(\d+)\s+(.*)$)" }
			{
			}
				...
				Main::MainServer&) override
			{
				...
				if (!targetSession)
				{
					...
					if (!scheduler.immediatePersist(std::source_location::current(),
						&Main::Persistence::PersistentDatabase::updateRoomCreationDisabledUntil, m_targetPlayerName, mutedUntil))
					{
				...
				else
				{
					...
					if (!targetSession->disableRoomCreation(m_durationDays))
					{
		...
		REGISTER_CMD(DisableRoomCreationFor, Common::Enums::PlayerGrade::GRADE_MOD)



		class EnableRoomCreationFor final : public ICommand
		{
		private:
			std::string m_targetPlayerName{};
		...
		public:
			explicit EnableRoomCreationFor(const Common::Enums::PlayerGrade requiredGrade)
				: ICommand{ requiredGrade, "/enableroomcreationfor <nickname>",  R"(^\S+\s+(.*)$)" }
			{
			}
				...
				Main::MainServer&) override
			{
				...
				if (!targetSession)
				{
					if (!scheduler.immediatePersist(std::source_location::current(), &Main::Persistence::PersistentDatabase::resetRoomCreationDisabledUntil,
						m_targetPlayerName))
					{
				...
				{
					if (!targetSession->enableRoomCreation())
					{
						session->sendMessage("error: unknown");
						return;
					}
		...
		REGISTER_CMD(EnableRoomCreationFor, Common::Enums::PlayerGrade::GRADE_MOD)
	};
}
